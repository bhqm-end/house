...
public class UserDao {
  ...
  public List<User> getUserList(User query) {
    //调用用户服务获取用户的list
    //ParameterizedTypeReference  用来做反序列换的class 返回泛型为RestResponse<List<User>
  ...
  public User getUserByToken(String token) {
    //如果不加"direct://"，我们会使用@LoadBalanced注解下的支持负载均衡的RestTemplate寻找userServiceName所对应的注册服务端口来进行访问
